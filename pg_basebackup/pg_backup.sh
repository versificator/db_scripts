...
function perform_backups()
{
        ...
        echo -e "\n\nPerforming backup via pg_basebackup"
        echo -e "--------------------------------------------\n"
 
        if [ "$ENABLE_PG_BASEBACKUP" = "yes" ]
        then
                echo "Globals backup"
                mv $BACKUP_DIR".done" $BACKUP_DIR".archive"
 
#                if ! pg_dumpall -g -w -h "$HOSTNAME" -U "$USERNAME" | gzip > $FINAL_BACKUP_DIR"globals".sql.gz.in_progress; then
                if ! pg_basebackup -x -h "$HOSTNAME" -U "$USERNAME" -D $BACKUP_DIR".in_progress"; then
                        echo "[!!ERROR!!] Failed to execute pg_basebackup" 1>&2
                else
                        mv $BACKUP_DIR".in_progress" $BACKUP_DIR".done"
# Delete daily backups 7 days old or more
                        find $ARCHIVE_DIR -maxdepth 1 -mtime +$DAYS_TO_KEEP -exec rm -rf '{}' ';'
                        rm -r $BACKUP_DIR".archive/"                        	
                fi
        else
	        echo "None"
        fi
